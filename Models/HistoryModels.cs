...
namespace orderAPi.Models
{
    public class HistroyClass
    {
        public List<Dictionary<string, object>> GetSearchModels(string clientinfo, string deviceinfo, string dateinfo, string cuurip)
        {
            ...
            foreach (DataRow dr in new database().checkSelectSql("mssql", "eatingstring", "exec eat.historycheckout @newid,@stdate,@endate;", dbparams).Rows)
            {
        ...
        public List<Dictionary<string, object>> GetStatistModels(string clientinfo, string deviceinfo, string cuurip)
        {
            ...
            for (int i = 4; i >= 0; i--)
            {
                ...
                items.Add(new Dictionary<string, object>() { { "statist", new Dictionary<string, object>() { { "key", yearMonth }, { "datas", int.Parse(new database().checkSelectSql("mssql", "eatingstring", "exec eat.statistcheckout @newid,@year;", dbparams).Rows[0]["itemcount"].ToString().TrimEnd()) } } }, { "colors", new int[] { 255, 0, 0, 1 } } });
            }
            return items;
        }
        ...
        public List<Dictionary<string, object>> GetAnalysisModels(string clientinfo, string deviceinfo, string cuurip)
        {
            ...
            dbparams.Add(new dbparam("@newid", new sha256().encry256($"{clientJson["clientid"].ToString().TrimEnd()}{randomJson["random"].ToString().TrimEnd()}{clientJson["accesstoken"].ToString().TrimEnd()}")));
            List<Dictionary<string, object>> items = new List<Dictionary<string, object>>();
            foreach (DataRow dr in new database().checkSelectSql("mssql", "eatingstring", "exec eat.analysischeckout @newid;", dbparams).Rows)
            {
